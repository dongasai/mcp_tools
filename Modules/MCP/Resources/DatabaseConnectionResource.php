...
class DatabaseConnectionResource
{
    ...
    private function getConnectionStats(int $connectionId, int $agentId): array
    {
        ...
        $totalQueries = \Modules\Dbcont\Models\SqlExecutionLog::where('database_connection_id', $connectionId)
            ->where('agent_id', $agentId)
            ->where('created_at', '>=', $thirtyDaysAgo)
            ->count();

        $successfulQueries = \Modules\Dbcont\Models\SqlExecutionLog::where('database_connection_id', $connectionId)
            ->where('agent_id', $agentId)
            ->where('success', true)
            ->where('created_at', '>=', $thirtyDaysAgo)
            ->count();

        $avgExecutionTime = \Modules\Dbcont\Models\SqlExecutionLog::where('database_connection_id', $connectionId)
            ->where('agent_id', $agentId)
            ->where('success', true)
            ->where('created_at', '>=', $thirtyDaysAgo)
            ->avg('execution_time');

        $lastQuery = \Modules\Dbcont\Models\SqlExecutionLog::where('database_connection_id', $connectionId)
            ->where('agent_id', $agentId)
            ->latest()
            ->first();
